...
import image from '../../assets/graphics/sprite.png';
...
const Canvas = ({ ...props }) => {
  ...
  let gameWorld: World | null = useRef(null).current;

  const loop = (ctx: CanvasRenderingContext2D, ctx2: CanvasRenderingContext2D) => {
    if (!canvasRef.current) return;
    if (gameWorld !== null) gameWorld.render();
  ...
  const onKeyDown = (event: React.KeyboardEvent) => {
    ...
    if (gameWorld) gameWorld.controll(activeKeys.current);    
  };
