...
import { ControlKey, Tank, TypeTank } from '../Models/Tank';
...
class World implements IWorld {
  ...
  init() {
    ...
    this.playerTank_1 = new Tank(this.observer);
    // this.playerTank_1 = null;
    ...
    if (this.bullets.length > 0) {
      for (const bullet of this.bullets) {
        if (!bullet.isExplose) {
          ...
          respawn({ img: this.img, ctx: this.ctx, tank: TypeTank.PLAYER1 });
          this.observer.emit('tank:shot', { str: 'tank shot' });
          const { clear: clBullet } = { ...bullet.prepareRenderBullet(this.img) };
          this.ctx.clearRect(...clBullet);
